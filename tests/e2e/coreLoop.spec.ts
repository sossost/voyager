...
test('코어 루프: 온보딩 → 탐사 → 조우 → 도감 → 워프 → 새로고침 복원', async ({ page }) => {
  ...
  // 6) 항성계 이탈(우주선 뷰) → 은하 전도 → 별 클릭(실제 마우스 — 화면공간 피킹) → 항행 → 도착
  await page.getByRole('button', { name: '← 항성계 이탈' }).click()
  ...
  if (await warpButton.isVisible()) {
    ...
    // 현재 별을 클릭했다면 항성계 진입으로 갈음
    await page.getByRole('button', { name: '항성계 진입' }).click()
  }
  await expect(page.getByRole('button', { name: '← 항성계 이탈' })).toBeVisible({ timeout: 8_000 })
