...
/** 은하에서 첫 블랙홀 하나 — 함교 스캔(exotic-scan) 검증용. SOL에서 바깥으로 훑는다. */
function findBlackHole(seed: Seed): StarId {
  const RANGE = 40
  for (let dsx = -RANGE; dsx <= RANGE; dsx++) {
    for (let sy = -5; sy <= 5; sy++) {
      for (let dsz = -RANGE; dsz <= RANGE; dsz++) {
        for (const star of starsInSector(seed, {
          sx: SOL_SECTOR.sx + dsx,
          sy,
          sz: SOL_SECTOR.sz + dsz,
        })) {
          if (star.kind === 'black_hole') return star.id
        }
      }
    }
  }
  throw new Error('블랙홀을 찾지 못했습니다')
}
const BLACK_HOLE = findBlackHole(LIFE1_SEED)
...
/**
 * 능동 스캔 (exotic-scan): 항법뷰에서 탐색을 발동하면 근처 블랙홀이 scannedStars에 기록되고
 * 감지 토스트가 뜬다. 마커는 일시적이라 이동하면 비워진다(맵 난잡 방지).
 */
test('항법 스캔: 탐색 버튼 → 근처 블랙홀 감지 + 이동 시 마커 소멸', async ({ page }) => {
  await page.goto('/?seed=LIFE1')
  await page.getByRole('button', { name: '이 우주로 출발' }).click()
  await expect(page.getByRole('button', { name: '도감' })).toBeVisible()
  await page.waitForFunction(() => window.__gameStore != null)

  // 블랙홀에 정박 — 스캔 반경 안에 최소 자기 자신이 든다
  await page.evaluate((target) => window.__gameStore?.getState().warpTo(target), BLACK_HOLE)
  await page.waitForFunction(
    (target) => {
      const state = window.__gameStore?.getState()
      return (
        state?.currentStarId === target && state.scene.kind === 'galaxy' && state.scene.view === 'ship'
      )
    },
    BLACK_HOLE,
    { timeout: 10_000 },
  ...
  // 항법(퍼스펙티브) 전환 → 탐색 버튼은 항법 전용 → 발동 → 감지 토스트 + scannedStars 기록
  await page.getByRole('button', { name: '▦ 항법' }).click()
  await page.getByRole('button', { name: '주변 탐색 — 근처 특이 천체 감지' }).click()
  await expect(page.getByText(/특이 천체 \d+개 감지/)).toBeVisible()
  await page.waitForFunction(
    (target) => window.__gameStore?.getState().scannedStars.has(target) === true,
    BLACK_HOLE,
    { timeout: 5_000 },
  )

  // 마커는 일시적 — 다른 별로 이동하면 스캔 기록이 비워진다 (맵 난잡 방지)
  await page.evaluate((target) => window.__gameStore?.getState().warpTo(target), WARP_TARGET)
  await page.waitForFunction(() => window.__gameStore?.getState().scannedStars.size === 0, undefined, {
    timeout: 10_000,
  })
})
