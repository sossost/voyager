...
- **벌지 내부 보정** — 벌지 안(반경 <10섹터, 가중 선형)에서는 물리 그대로면 하늘 전체가 "모래폭풍 워시"가 된다(실측 기각). 게임 미학 우선(결정 28): 안쪽일수록 광량을 원반면 띠로 모으되 고고도 잔광 0.25는 남긴다. 벌지 밖 정박(돔 뷰)에는 영향 0.
- 실린더 절반 높이 1,700→2,600 (외곽 정박의 벌지 돔 ±10°가 페이드 여유를 갖고 들어가게), 포화 상한 26→30.
