...
**Consequences:** 렌더·DOM 전용 — GEN_VERSION·저장 무관. E2E는 버튼 텍스트 기준이라 무영향(콜아웃 안의 항행/별계 진입 그대로).
...
- **베이크 캐시**: 같은 정박 별이면 씬 전환(은하↔별계) 리마운트에 재베이크하지 않는 단일 캐시(DistantGalaxies와 같은 트레이드오프).
