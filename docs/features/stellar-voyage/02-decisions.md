...
**Consequences:** 주소창이 항상 공유 가능한 상태가 된다(바 진입도 `?seed=`로 자동 보강). 검증: `systemUrl` 단위 18케이스(파싱·빌드·resolve·jsdom 동기화) + E2E 딥링크 복원·URL 동기화 단언. 한계: 자신의 우주(프로필)가 있는 플레이어가 **남의** 딥링크를 열면 자기 우주가 로드되고 URL이 자가 보정된다 → 이 무음 실패를 결정 45(게스트 둘러보기)가 해소.

---

### 45. 공유 우주 게스트 둘러보기 — 시드 충돌 시 기록 비파괴 미리보기 (결정 44 보완, 2026-06-16)

**문제:** 결정 44의 한계 — 이미 자기 우주(시드 A)를 저장한 플레이어가 **다른 시드(B)의** 공유 딥링크를 열면, 링크가 *아무 안내 없이* 무시되고 자기 우주가 로드됐다(무음 실패). 브라우저당 우주(Profile id:1)가 하나뿐이라 생기는 충돌인데, 유저는 링크를 따라왔다는 사실조차 알 수 없었다. (신규 방문자=프로필 없음에겐 잘 동작 — 문제는 기존 플레이어 한정.)

**결정:** 부트에서 **URL seed ≠ 저장 seed**를 감지하면 충돌 프롬프트를 띄우고, "둘러보기"는 **기록을 저장하지 않는 게스트 세션**으로 공유 우주를 연다(내 우주 보존). "내 우주로 계속"은 링크를 무시하고 평소대로 진입.

| 열린 질문 | 결정 | 근거 |
|-----------|------|------|
| 충돌 처리 방식 | 게스트 둘러보기(기록 비파괴) | 사용자 선택 — 무음 실패/기록 대체/안내만 중에서 비파괴 미리보기 채택. |
| 게스트 저장 차단 | `guestMode` 플래그가 store의 `commit`(=persist 래퍼)을 no-op으로 | 모든 변이 액션이 write-through(set 먼저, commit 나중)라 in-memory 세션은 정상 동작하고 디스크 쓰기만 빠진다. |
| 게스트 하이드레이션 | 빈 기록 | 다른 우주라 내 방문·수집 기록이 무의미. 단 `seenHints`는 넘겨 온보딩 힌트 재노출 방지. |
| 출구 | 배너 "내 우주로 돌아가기" → 쿼리 없는 주소로 새로고침 | BootGate가 저장된 내 프로필을 재복원 — 가장 단순·견고. |
| 신규 방문자(프로필 없음) | 변경 없음 | 충돌이 없으므로 공유 우주를 평소대로 채택(결정 44 그대로). |

**구현:** `createGameStore` `guestMode` 옵션 + `isGuestMode` 상태 + `commit` 게이트(persist 4곳) / `BootGate` `shared-universe` 부트 상태 + `startGuestSession` / `SharedUniversePrompt`(충돌 다이얼로그, 별 이름 표시) / `GuestModeBanner`(상시 코션 + 출구, HudLayer 마운트). 렌더/상태/URL 전용 — 저장 포맷·GEN_VERSION 무관.

**Consequences:** 무음 실패 제거 — 기존 플레이어도 공유 블랙홀 등을 자기 세이브 손실 없이 본다. 게스트 진행(워프·탐사·수집)은 휘발 — 새로고침(클린 URL) 시 사라지고 내 우주 복원. 검증: 게스트 단위 4케이스(플래그·워프 시 디스크 미기록·일반 모드 대조) + E2E 충돌 프롬프트→둘러보기→복귀. 비범위: 게스트 우주를 영구 채택(우주 전환)·다중 세이브 슬롯.

---

### 46. 블랙홀 렌즈 후방 컬링 + 은하수 띠 거리 페이드 (블랙홀 버그 2건, 2026-06-16)

**문제(사용자 지적, 공유 블랙홀 VOYAGER `-2:-1:1:2`에서 발견):**
1. **팬텀 그림자** — 함교(1인칭) 뷰에서 블랙홀 반대편(시선 ~180°)을 보면, 블랙홀과 별개로 검은 구가 하나 더 떠 보였다. 원인: `CurrentSystem`이 매 프레임 `blackHoleLens`를 게시할 때 `dist < LOD`만 보고 활성화 → 블랙홀이 카메라 뒤일 때도 활성. `Vector3.project()`는 카메라 뒤(w<0)에서 화면 안쪽으로 뒤집힌 UV를 내, 레이마칭이 그 자리에 그림자를 그렸다.
2. **항법 뷰 은하수 부재** — 함교에 보이는 황색 은하수 배경(`ShipViewGalaxyGlow` 원반 밴드, 결정 38)이 퍼스펙티브(항법)엔 안 떠 블랙홀 배경이 비어 보였다. 단, 무조건 마운트하면 **줌아웃 은하 조망 시 별 중심 띠가 둥 떠 부자연스럽다**(사용자 지적).

**결정:**
1. 렌즈 활성 조건에 **정면 판정** 추가 — `(bhPos − camPos)·forward > 0`(시선·BH 방향 내적). 뒤편이면 `clearBlackHoleLens()`. project()는 이 가드 뒤에서만 호출.
2. `ShipViewGalaxyGlow`를 모든 뷰에 마운트하되 **밴드 불투명도를 카메라-별 거리로 페이드**(NEAR 280 / FAR 1,100, smoothstep). 함교(≈63u)·항법 근거리는 NEAR 안이라 그대로 보이고, 줌아웃하면 걷힌다. 결정 38(ship 전용)을 "거리 게이트"로 개정.

**Consequences:** 렌더/카메라 전용 — 저장·GEN_VERSION 무관. 검증: preview(M4 Pro 실GPU) 직접 확인 — ① 180° 회전 시 팬텀 소멸(별밭만), ② 항법 근거리 띠 표시 / 줌아웃 시 은하 조망 깨끗. 코어 글로우는 기존 페이드(은하중심 거리) 유지. [[blackhole-visual-iteration]]
