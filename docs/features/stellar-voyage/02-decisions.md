...
**Chosen:** B — 세 갈래 동시 변경:
1. **엔진** (`engine/math/trig.ts` 신설 + `density.ts`): cos/atan2를 사칙연산 유리 근사(Bhaskara I, 최대 오차 ~0.002)로 벤더링 — 결정 14의 "산술 연산만" 원칙 유지. 밀도 = 원반 감쇠² × 수직 감쇠² × 2팔 아르키메데스 나선(비틀림 0.45rad/섹터, 팔 바닥 0.15) × 덩어리 질감(바닥 0.5) + 벌지(반경 6섹터). `MAX_STARS_PER_SECTOR` 80 → 12.
...
**Consequences:** `tests/golden/universe.golden.test.ts.snap` 전면 재생성. 기존 v1 프로필은 부트 시 안내 모달(확인 완료). 섹터당 별 밀도 80 → 12 (최초 24로 갔다가 전수 렌더 전환 후 과밀 피드백으로 추가 하향 — 같은 GEN_VERSION 2, 출시 전). 구형 페이드는 결정 22에서 가상화와 함께 제거됨.
...
**Chosen:** B — 결정 12의 섹터 가상화는 '무한 우주' 가정이었으나, 은하는 유한하다(반경 48섹터, 총 ~1.7만 별). 전수 생성을 실측하니 이름 포함 ~50ms — 가상화가 해결하던 문제가 존재하지 않았다. `useGalaxyStars`가 시드당 1회 모듈 캐시로 전수 생성하고, `GalaxyStarField`가 1드로콜로 그린다. 피킹(결정 20)은 전체 별 대상 O(n) — 클릭 시점에만 수행하므로 ~1.7만 × 투영 ≈ 수 ms. 별 셰이더는 '단단한 코어 + 옅은 헤일로' 프로파일로 블러 없이 또렷하게, 크기 하한 1.2px로 원거리에서도 소멸하지 않는다. 백드롭은 opacity 0.35의 보조 성운 색감으로 격하.
