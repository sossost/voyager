...
**은하수 띠 (피드백 "태양계 궤도에서 우주를 바라볼 때 어떻게 보이겠어?" 후속):** 은하 안에서는 은하의 "형태"가 아니라 ①분해되는 가까운 별(점) + ②분해되지 않는 먼 별빛의 띠(은하수)가 보인다. `scenes/system/GalacticBand`가 ②를 맡는다 — 실거리 1,200 밖 별의 광량(1/d², 별당 상한 0.06)을 등장방형 256×128에 3×3 텐트 커널로 누적·톤맵한 광량 맵이 단일 출처. **은하수는 구름이 아니라 입자다** ("뿌연 구름같다" 피드백으로 텍스처 단독안 기각): 이 맵을 중요도 샘플링(CDF)한 입자 별 1.4만 개(Points 1드로콜, 결정론 해시)가 띠의 본체이고, 같은 맵의 텍스처 구(반경 3,900, BackSide)는 불투명도 0.12의 밑광만 깐다. 등장방형 매핑이 three.js SphereGeometry UV 공식과 동일해 점 별 셸과 방향이 정확히 정합한다.
**Consequences:** 렌더 전용 — 시드 결정론 유지, GEN_VERSION 무관. 베이크는 starId당 1회(플래시 뒤 마운트 프레임에서 수행), 드로콜 +2 (밑광 구 + 입자 Points).
