...
**코어 근처 "형광등 줄" 수정 (같은 날, 결정 38 톤매핑 보완):** 코어 근처 정박에서 거대한 근거리 광이 좁은 띠 프라이어(σ 650)에 눌려 넓은 워시 위에 또렷한 흰 줄로 떴다(실측 기각). 세 가지로 해소:
1. 프라이어 완화 — σ 650→1,000, 바닥 0.22→0.3: 워시와 띠가 한 몸의 경사로 이어진다.
2. **하이라이트 소프트 니** — 선형 클램프 대신 1-exp 곡선(t=1 정규화): 밝은 구간이 평평한 흰 띠로 깎이지 않고 부드럽게 눌린다. 니가 저·중광량을 들어 올리므로 감마는 1로 환원(겹치면 중간 워시가 떠서 팔 사이 골이 "세로 기둥"으로 도드라진다 — 실측).
3. 합성 블러 2→4px — 코어 근처는 좁은 방위각이 화면을 채워 격자 블록이 드러난다.
