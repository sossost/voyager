...
**Chosen:** B — `scenes/shared/DistantGalaxies`. 셸 반경 16,000~23,000(최대 줌아웃 6,000 밖, 카메라 far 30,000 안)에 고정 방향 8개. 그룹이 매 프레임 카메라 쿼터니언을 복사(빌보드)하고 자식 메시가 면내 회전·타원 스케일을 가진다. 좌표는 은하 중심 기준이며 별계 씬(플로팅 오리진, 결정 15)은 현재 별의 월드 좌표만큼 역오프셋해 마운트 — 어느 씬에서 보든 정확히 같은 하늘 방향에 떠 있다(우주 일관성, SystemBackdropStars와 같은 기준계).
...
**Chosen:** B — `scenes/system/SystemBackdropStars`. 별계 씬은 플로팅 오리진(결정 15)이므로 현재 별 기준 상대 벡터를 고정 반경 4,000 셸로 정규화 — 가장 가까운 이웃(수십 유닛)도 행성 궤도(~53)와 절대 겹치지 않고 시차도 없다. 밝기는 실거리 기반 감쇠((150/d)^0.8)에 좌표 파생 지터를 곱한 뒤 하한 0.04로 클램프 — 가까운 이웃은 또렷, 은하 건너편은 띠로 뭉친다. 점 크기 캡 4px — 항성·행성이 주인공. 별 개성 해시는 `scenes/shared/starVariance`로 단일화 — 은하 지도와 배경 하늘이 같은 변주를 공유한다(모두가 같은 하늘).
**Consequences:** 렌더 전용 — 시드 결정론 유지, GEN_VERSION 무관.

---

### 26. 워프 연출 다듬기 (백로그 E-2, 결정 16 보완)

**Date:** 2026-06-11 (비주얼 2차 패스 — "현 3단 타임라인이 투박함")

결정 16의 3단 타임라인 구조·상수(WARP_*_MS)·총 길이는 불변 — 스테이지 A/B의 실행 품질만 다듬는다:
- **스트리크 2층화**: 쐐기형 단층 레이 → 버킷 내 감쇠의 가는 빛줄기 2층(가늘고 빠른 한색 주층 110 + 굵고 느린 보랏빛 보조층 64). 레이별 속도·꼬리 길이·두께 변주.
- **중앙 글로우**: 진행 막판에만 차오르는 글로우가 플래시(스테이지 B)로의 전환을 잇는다 — 백색이 무에서 튀지 않는다.
...
**Consequences:** 렌더 전용 — GEN_VERSION 무관. CSS(global.css .warp-flash) 지속시간은 warpTimeline.ts 상수와 수동 동기화 관계 유지.
