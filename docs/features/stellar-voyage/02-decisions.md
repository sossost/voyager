...
**Chosen:** B — `scenes/galaxy/galaxyNebulae.ts`: 성운 11개를 **결정론 거부 샘플링**(해시 후보 → sectorDensity ≥ 0.12만 수락)으로 나선팔 위에만 배치 — "아무 데나 뿌린 구름"(결정 28 기각 패턴)이 아니라 은하 구조에 속한 가스. 은하 매크로 형상이 시드 무관이므로 성운도 시드 무관 — 모든 플레이어가 같은 하늘.
- **팔레트**: 로즈(발광)·청록(반사 — UI 홀로 색 언어)·보라(이온화). 밴드 한색/벌지 난색과 안 겹쳐 "성운"으로 분리돼 읽힌다.
- **파노라마 통합**: 광선마다 가우시안 선적분 닫힌형(√π·σ·exp(-(d⊥/σ)²)) — 샘플링 없이 광선당 성운 수만큼의 산술. 노출은 밴드 자동 노출과 분리된 고정 기준(정박 위치 따라 성운 밝기가 출렁이면 안 됨), 가장자리 페이드만 공유.
- **지도 통합**: GalaxyNebula 베이크에 같은 좌표 규약((sx+R)·U, (sz+R)·U)으로 라디얼 그라디언트 — 함내 하늘에서 본 그 성운이 지도의 같은 자리에 뜬다.
- 실측 기각: σ 1.6~4.2 + 레벨 0.55는 띠 전체를 물들이는 워시 → σ 1.0~2.4 + 레벨 0.32로 "군데군데 패치" 스케일 확정.
**Consequences:** 렌더 전용 — 생성 분포·GEN_VERSION·저장 무관. 성운은 장식이며 피킹·탐사 대상 아님 (게임플레이 성운은 백로그 G-c-9 이색 천체의 몫).
