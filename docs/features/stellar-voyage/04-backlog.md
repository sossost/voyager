...
## F. 비주얼 3차 — 천체 디테일 + 위치 가시화 (2026-06-11 합의, 다음 작업)
...
### F-1. 항성·행성 디테일 — 단색 구 탈피

- **항성** (`SystemScene.tsx`의 meshBasicMaterial 구 대체): 절차 셰이더 — 시간 애니메이션 노이즈 입상반(끓는 표면) + 림 다크닝 + 코로나 글로우(가산 빌보드). 색·온도는 `SPECTRAL_RENDER` 재사용, 고티어 Bloom이 증폭. 예상 반나절.
- **행성** (`Planet.tsx`): paletteSeed 결정론 **CanvasTexture 베이크** (등장방형, GalaxyNebula·GalacticBand에서 검증된 패턴, 계당 1~8장 마운트 시 ~수 ms). 암석형 = 노이즈 대륙·지형 색 밴드 + 극관, 가스형 = 위도 밴드 + 소용돌이 변형. 공통 = 자전(useFrame), 프레넬 대기 림 글로우. 생명체 행성 = 구름층 + 청록 대기 강화. 조명은 기존 meshStandardMaterial + 항성 포인트라이트 유지(낮/밤 경계 공짜). 예상 1~2일.

### F-2. 방문/현재 위치 가시화 — 정보 위계: 현재 > 선택 > 방문 > 미방문

현 상태: 방문 = 파란 빌보드 링(InstancedMesh, 캡 512), 현재 = 강조 링, 선택 = 보라 펄스 링 — 7.3천 별 줌아웃에서 묻힘.

- **기본 ① 현재 위치 비콘**: 줌 무관 화면 크기 클램프 펄스 링 + 소나처럼 퍼지는 확장 링, 강한 액센트 색 — 어떤 줌에서도 "여기"가 한눈에.
- **기본 ② 방문 별 발광 틴트**: 링 대신 `GalaxyStarField`의 starColor attribute를 방문 시 갱신 — 방문한 별 자체가 청록빛으로 "켜진" 느낌. 줌아웃 가시성 + 마커 512 캡 해소. (`VisitedStarMarkers` 대체 또는 축소)
- **토글 ③ 여정 경로선**: Visit 타임라인 순서를 잇는 폴리라인, 오래된 구간 페이드. HUD 토글 버튼 (취향 타는 요소라 기본 off 고려).
- **후순위 ④ 오프스크린 화살표**: 현재 별이 화면 밖이면 가장자리 방향 화살표 (DOM HUD에 투영 — drei Html 금지 규칙 유의).
