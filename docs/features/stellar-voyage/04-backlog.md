...
> **🔝 우선순위 격상 (2026-06-15).** 엔진 확장(쌍성계 / 이색 천체)을 다음 작업으로. 둘 다 출력 분포가 바뀌므로 **`/yc:brainstorm` → GEN_VERSION 범프 + 골든 재생성**이 선행. 철칙 2·3(GEN_VERSION 규칙·draw append-only) 숙지 필수.
> - ✅ **쌍성계 = GEN_VERSION 4** 완료 (2026-06-15) — E-5#3 참조.
> - ⏭️ **이색 천체 = GEN_VERSION 5** (다음) — 아래 G-c-9.
...
9. **🔝 이색 천체: 블랙홀·성운·펄서 등** — 별 분포·종류 변경 = **GEN_VERSION 5** + 골든 재생성 (쌍성계가 4를 가져감). 표현(블랙홀 렌즈 효과·펄서 점멸·성운 볼류메트릭)과 게임플레이(탐사 보상? 항행 위험?)가 모두 열려 있음 — `/yc:brainstorm` 선행 필수.
