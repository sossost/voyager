...
2. ~~**워프 연출 다듬기**~~ — ✅ 완료 (2026-06-11, 결정 26): 워프 카메라 리그(현 위치 → 목표 응시 → 돌진) + 혜성형 트레일 2층 + 점화 시차 + 중앙 글로우 + FOV 큐빅 서지 + 플래시 청백 틴트, 총 ~2.7s로 연장. 타임라인 구조 불변.
...
6. ~~**항성계 배경 별**~~ — ✅ 완료 (2026-06-11, 결정 25): `scenes/system/SystemBackdropStars` 실제 이웃 별 셸 투영 + `GalacticBand` 은하수 띠(분해 안 되는 먼 별빛의 확산 글로우), 시드 결정론 유지.
