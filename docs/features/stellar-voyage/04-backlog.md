...
3. **GalaxyBackdrop uPixelRatio** — `useEffect` 설정이라 리사이즈 후 다음 렌더까지 이전 값. SectorPoints와 패턴 통일 (마운트 1회 + 조건부 갱신).
4. **CameraRig 카메라/타깃 비동기** — focus 변경 시 `controls.target`과 `camera.position`을 동일 프레임에 일괄 적용.
