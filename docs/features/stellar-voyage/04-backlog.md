...
### J-1. 우주선 뷰 — 캐노피 프레임 + 함교 분위기 강화

- **캐노피 오버레이** — 화면 가장자리에 SF 창틀 프레임 SVG/CSS 오버레이. 모서리 브래킷 + 측면 패널 라인으로 "창밖을 내다보는" 느낌 부여. 현재 결정 35 콘솔 스킨과 레이어 분리.
- **주변부 비네팅** — 화면 가장자리 청색 틴트 비네트 (CSS radial-gradient). 우주의 어둠감 강조.
- **HUD 요소 엣지 정렬** — 뷰 전환 버튼·여정경로 토글 등을 화면 코너/엣지에 정렬. 중앙부는 우주 시야 확보. Mass Effect식 좌하단 상태 클러스터 참고.
- **스캔라인/글리치 질감** — 콘솔 스킨에 미세 스캔라인 패턴(CSS `repeating-linear-gradient` + 낮은 opacity) 추가. 홀로그램 느낌 강화.

### J-2. 공통 HUD — 버튼·패널 정제

- **버튼 계층 정리** — 1차 액션(워프·탐사), 2차 액션(뷰 전환·토글)을 시각적 무게감으로 구분. 현재 모든 버튼이 동일한 스타일.
- **콜아웃 패널 타이포·여백** — 별/행성 정보 패널의 폰트 크기·행간·내부 여백 재조율. 현재 정보 밀도가 약간 낮거나 불균형.
- **토스트 위치** — 현재 위치 확인 필요. 캐노피 프레임과 겹치지 않도록 safe zone 설정.
- **퍼스펙티브 뷰 HUD** — 우주선 뷰와 동일 HUD를 쓰지만, 퍼스펙티브에선 캐노피 프레임 숨김 + 레이더/맵 느낌 UI로 전환하는 방안 검토.

### J-3. 구현 순서 (권장)

1. 캐노피 프레임 CSS 오버레이 — 가장 임팩트 크고 범위 한정
2. HUD 버튼 계층 정리 + 엣지 정렬
3. 콜아웃 패널 타이포·여백 재조율
4. 스캔라인 질감 (마지막 — 취향 영역, 실측 후 결정)
