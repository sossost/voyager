...
**충돌 방지 튜닝 (2026-06-15 사용자 피드백 — 별/행성 관통):** 별 시각 반경(3)이 궤도 스케일 대비 커서, 항상-circumbinary로 바꾼 뒤 ① 가까운 쌍이 서로 관통하고 ② 멀리 도는 별이 행성 궤도를 가르는 문제 발생. 렌더에서 3중 방어 (모두 `multiplicity.ts`, GEN_VERSION 무관):
> - **반장축 하한** — `pairSemiMajor`가 근점거리 ≥ 두 별 표면 + 간격(2.4)이 되도록 aTotal에 하한. 편심 근점에서도 안 겹친다.
> - **렌더 편심 축소** — `ECC_RENDER_FACTOR=0.4`로 시각 편심만 줄여 근점 충돌 완화(생성 편심은 보존).
> - **행성 궤도 밀어내기** — `planetClearanceOffset(star)`가 별 군집 최대 반경(apoapsis 기준 보수적 상한)을 구해 행성 궤도를 그만큼 바깥으로 민다. `orbitRadiusOf`·`planetOrbitPosition`에 offset 인자를 추가해 CurrentSystem·Planet·PlanetCalloutProjector가 공유.
>
> 검증(G+G+G 삼중성, 최악 케이스): 별 표면 간격 ≥ 2.4(전 위상), 행성 여유 9.16 — 관통 없음. 삼중성은 군집이 커 행성이 다소 멀어지는(빈 띠) 절충이 있으나 관통 0 우선.
