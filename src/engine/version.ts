 ...
 * v12 (2026-07-10): 유니크 항성계 핀 (exotic-codex — docs/features/exotic-codex):
 *   아케론(disk_bh, 섹터 14,0,16)을 SOL_STAR 핀 방식으로 고정 배치 — 해당 섹터의 인덱스 0
 *   별이 상수로 대체된다(다른 별·RNG 스트림 무영향). 유니크계는 planetsOf가 빈 배열
 *   (대질량 쌍성 + 초신성의 행성계 파괴). 카리브디스(feeding_bh)는 구현 완료 후 백업 —
 *   레지스트리 제외로 미생성, 퀄리티 보강 후 재투입 예정(그때는 새 범프 필요: 핀 추가 =
 *   해당 섹터 출력 변경). 절차 블랙홀의 강착원반 제거(암흑화)는 렌더 전용이라 이 범프와
 *   무관. 프로브 섹터(2,0,3) 무영향 — 골든은 genVersion 필드만 변경. Sol·LIFE1 무영향.
 */
