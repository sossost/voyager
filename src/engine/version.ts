 ...
 * v12 (2026-07-10): 유니크 항성계 핀 2개 (exotic-codex — docs/features/exotic-codex):
 *   아케론(disk_bh, 섹터 14,0,16)·카리브디스(feeding_bh, 섹터 -2,0,30)를 SOL_STAR 핀 방식으로
 *   고정 배치 — 해당 두 섹터의 인덱스 0 별이 상수로 대체된다(다른 별·RNG 스트림 무영향).
 *   유니크 2계 모두 planetsOf가 빈 배열(카리브디스: 질량 이전 환경, 아케론: 대질량 쌍성 +
 *   초신성의 행성계 파괴). 절차 블랙홀의 강착원반
 *   제거(암흑화)는 렌더 전용이라 이 범프와 무관. 프로브 섹터(2,0,3) 무영향 — 골든은 genVersion
 *   필드만 변경. Sol·LIFE1 시작계 무영향 (핀 섹터가 멀리 떨어져 있음).
 */
