 ...
 * (참고) 유니크 항성계 핀(exotic-codex)은 구현 후 전체 백업 — 레지스트리(UNIQUE_SYSTEMS)가
 *   비어 있어 생성 출력 무변경, 범프 불필요. 재투입 시 새 범프 필수 (핀 추가 = 섹터 출력 변경).
 *
 * v12 (galaxy-realism-pass — 원반 두께 고증):
 *   수직 프로파일 교체 — 구 렌즈형 테이퍼(중심 5 → 림 1.2섹터)를 "원반 상수 두께(1.5섹터,
 *   Jurić et al. 2008 스케일 하이트) + 벌지만 볼록(중심 5섹터)"으로. 중간 반경 두께비가
 *   19:1 → 32:1이 되어 측면 실루엣이 실은하와 정합한다 (사용자 승인 — 기존 우주 재배치 감수).
 *   sy=0 평면 밀도는 완전 불변 — Sol·LIFE1 시작계·E2E 타깃·골든 프로브(sy=0) 전부 무영향.
 *   sy≠0 섹터의 별 수·위치만 변경 (고고도 별 감소).
 */
