...
/**
 * 별계 씬 균일 별밭 — 천구 전체에 고르게 흩어진 장식 별 (결정 28).
 *
 * 물리 충실(은하수 띠)은 "구름 같다"로 기각 — 스텔라리스식 게임 미학을 따른다:
 * 배경은 균일하고 차분해야 전경(항성·행성)이 산다. 실제 이웃 별
 * (SystemBackdropStars)이 이 위에 밝은 액센트로 얹힌다.
 * 결정론 해시 기반 — 시드 무관 장식이라 모든 플레이어가 같은 하늘을 본다.
 */

/** 별 수 — 빈틈없이 고르되 과밀하지 않게. */
const STARFIELD_COUNT = 6_000
/** 천구 반경 — 실이웃 별 셸(4,000) 바깥 (가산이라 순서 무관, far 안). */
...
const SIZE_BASE = 1.1
const SIZE_SPAN = 1.4
/** 점 크기 캡 — 배경은 풍경, 실이웃 별보다 도드라지면 안 된다. */
...
function buildStarfieldGeometry(): BufferGeometry {
  ...
  for (let star = 0; star < STARFIELD_COUNT; star++) {
    ...
    positions[star * 3] = Math.cos(azimuth) * horizontal * STARFIELD_RADIUS
    positions[star * 3 + 1] = y * STARFIELD_RADIUS
    positions[star * 3 + 2] = Math.sin(azimuth) * horizontal * STARFIELD_RADIUS
...
export function SystemStarfield() {
  const geometry = useMemo(() => buildStarfieldGeometry(), [])
